...
export const AddTreatmentModal: React.FC<AddTreatmentModalProps> = ({ isOpen, onClose, onSave }) => {
  ...
  const [pendingItems, setPendingItems] = useState<any[]>([]); // Lista de medicamentos por revisar
  const [currentIndex, setCurrentIndex] = useState(0); // Índice del medicamento actual
  ...
  // Cada vez que cambia el medicamento en revisión, actualizamos el formulario
  useEffect(() => {
  ...
  const nextStep = () => {
    // Si quedan más en la lista, pasamos al siguiente
    if (currentIndex < pendingItems.length - 1) {
      setCurrentIndex(prev => prev + 1);
    } else {
      // Si era el último, limpiamos y cerramos
      setPendingItems([]);
      onClose();
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
      <div className="bg-white rounded-[2.5rem] w-full max-w-md shadow-2xl overflow-hidden border border-white animate-in zoom-in-95 duration-200">
        ...
        {/* Header con progreso si hay varios */}
        ...
        <div className="p-6 space-y-5 max-h-[85vh] overflow-y-auto">
          ...
          <form onSubmit={handleSaveCurrent} className="space-y-5">
            {/* Solo mostramos el escáner si no estamos revisando una lista */}
            {pendingItems.length === 0 && (
              <>
            ...
            {/* Campos de edición detallada (Aparecen siempre para que puedas modificar lo que la IA leyó) */}
            <div className="space-y-4">
              <div>
                ...
                <input type="text" required value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full px-5 py-4 rounded-2xl bg-slate-50 border-none focus:ring-2 focus:ring-blue-500 outline-none text-sm font-bold text-slate-700 shadow-inner" />
              </div>
              <div>
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Instrucciones / Dosis</label>
                <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} rows={3} className="w-full px-5 py-4 rounded-2xl bg-slate-50 border-none focus:ring-2 focus:ring-blue-500 outline-none text-sm font-medium text-slate-600 resize-none shadow-inner" />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select value={formData.frequencyHours} onChange={(e) => setFormData({ ...formData, frequencyHours: parseInt(e.target.value) })} className="w-full px-5 py-4 rounded-2xl bg-slate-50 border-none text-sm font-bold text-slate-700 appearance-none shadow-inner">
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input type="number" min="1" value={formData.durationDays} onChange={(e) => setFormData({ ...formData, durationDays: parseInt(e.target.value) })} className="flex-1 px-5 py-4 rounded-2xl bg-slate-50 border-none text-sm font-bold text-slate-700 shadow-inner" />
            ...
            <div className="flex gap-3 pt-4">
              {pendingItems.length > 0 ? (
                ...
                <>
                  ...
                  <button type="submit" className="flex-[2] py-4 bg-slate-900 text-white rounded-[1.5rem] font-bold shadow-xl active:scale-95 transition-all">Guardar Tratamiento</button>
                </>
              )}
            </div>
          </form>
