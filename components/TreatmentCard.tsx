...
import { Check, X, Clock, Pill, Droplets, AlertCircle, Zap } from 'lucide-react';
...
export const TreatmentCard: React.FC<TreatmentCardProps> = ({
  ...
  const getColors = () => {
    if (isOverdue) return { bg: 'bg-rose-50', border: 'border-rose-200', icon: 'bg-rose-100 text-rose-600', btn: 'from-rose-500 to-red-600' };
    if (isDueSoon) return { bg: 'bg-amber-50', border: 'border-amber-200', icon: 'bg-amber-100 text-amber-600', btn: 'from-amber-500 to-orange-600' };
    if (treatment.type === 'medication') return { bg: 'bg-blue-50', border: 'border-blue-200', icon: 'bg-blue-100 text-blue-600', btn: 'from-blue-500 to-indigo-600' };
    return { bg: 'bg-emerald-50', border: 'border-emerald-200', icon: 'bg-emerald-100 text-emerald-600', btn: 'from-emerald-500 to-teal-600' };
  };
    ...
    <div className={`relative rounded-[2.5rem] overflow-hidden shadow-lg border-2 transition-all ${colors.bg} ${colors.border}`}>
      <div className="p-6">
        ...
        <div className="flex items-start gap-4 mb-4">
          <button 
            onClick={onEdit}
            className={`relative group flex-shrink-0 w-16 h-16 rounded-2xl flex items-center justify-center ${colors.icon} shadow-sm transition-transform active:scale-90`}
          ...
          <div className="flex-1 min-w-0">
            ...
            {/* Descripción multilínea y legible */}
            <p className="text-sm text-slate-600 font-medium leading-snug mt-1 italic">
